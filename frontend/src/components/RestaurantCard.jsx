...
export default function RestaurantCard({ restaurant, rank, originCoords, theme }) {
  ...
  const openColor =
    isOpen === true
      ...
      : { bg: 'rgba(255,255,255,0.08)', text: 'rgba(255,255,255,0.4)', dot: 'rgba(255,255,255,0.3)' };
  ...
  return (
    <motion.article
      ...
      transition={{ delay: rank * 0.09, duration: 0.52, ease: [0.22, 1, 0.36, 1] }}
      whileHover={{ y: -4 }}
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="font-body text-xs mt-0.5 leading-relaxed" style={{ color: 'rgba(255,255,255,0.38)' }}>
            {restaurant.address}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2.5 flex-wrap">
        {restaurant.rating && restaurant.rating !== 'N/A' && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
            ...
            <span className="font-body text-xs font-medium text-white/70">{restaurant.rating}</span>
          </div>
        )}
        <div
